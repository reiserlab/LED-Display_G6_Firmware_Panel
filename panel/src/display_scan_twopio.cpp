...
#if SPI_DIAG
// LAB-43 rare-burst hunt: per-row completion-poll duration instrumentation.
// A row whose wait_burst_done() runs longer than its neighbours (but under the
// 2 ms self-heal timeout, so scan_timeouts stays 0) holds that row driven the
// whole time → a localized bright burst. Thresholdless: track the worst single
// -row wait per heartbeat window (read-and-cleared each window) plus the worst
// ever, so a stall shows at ANY duty as rowmax >> the ~scan_us/20 per-row norm.
static volatile uint32_t rowmax_win_us  = 0;   // worst row wait this window
static volatile uint32_t rowmax_win_row = 0;
static volatile uint32_t rowmax_all_us  = 0;   // worst row wait ever
static volatile uint32_t rowmax_all_row = 0;
// win_* = worst since last call (then cleared); all_* = worst since boot.
void twopio_get_longrow(uint32_t &win_us, uint32_t &win_row,
                        uint32_t &all_us, uint32_t &all_row) {
    win_us = rowmax_win_us; win_row = rowmax_win_row;
    all_us = rowmax_all_us; all_row = rowmax_all_row;
    rowmax_win_us = 0; rowmax_win_row = 0;      // read-and-clear per window
}
#endif
...
#if SPI_DIAG
    uint32_t wb0 = time_us_32();
    bool wb_ok = wait_burst_done();
    uint32_t wb_us = (uint32_t)(time_us_32() - wb0);
    // Track the worst single-row completion wait (this row held driven the whole
    // time). Thresholdless: a stall shows as rowmax >> the per-row norm.
    if (wb_us > rowmax_win_us) { rowmax_win_us = wb_us; rowmax_win_row = (uint32_t)r; }
    if (wb_us > rowmax_all_us) { rowmax_all_us = wb_us; rowmax_all_row = (uint32_t)r; }
    if (!wb_ok) {
#else
    if (!wait_burst_done()) {
#endif
